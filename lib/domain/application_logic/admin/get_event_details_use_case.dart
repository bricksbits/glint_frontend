...
class GetAdminEventDetailsUsecase
    extends UseCase<AdminEventDetailDomainModel, void> {
  final AdminDashboardRepo adminDashboardRepo;

  GetAdminEventDetailsUsecase(this.adminDashboardRepo);

  @override
  Future<Stream<AdminEventDetailDomainModel>> buildUseCaseStream(
      void params) async {
    final StreamController<AdminEventDetailDomainModel> controller =
        StreamController();
    try {
      adminDashboardRepo.getEventDetailsForAdmin().then((result) {
        switch (result) {
